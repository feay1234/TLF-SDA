...
class DSTExample(object):
    ...
    def __init__(self,
        ...
        self.turn_weight = 1
        self.turn = int(self.guid.split("-")[-1])
        self.max_turn = 0
        self.refer_turn_label = refer_turn_label
    ...
    def __repr__(self):
        ...
        if self.text_a_label:
            s += ", text_a_label: %s" % str(self.text_a_label)
        if self.text_b_label:
            s += ", text_b_label: %s" % str(self.text_b_label)
        if self.history_label:
            s += ", history_label: %s" % str(self.history_label)
        if self.values:
            s += ", values: %s" % str(self.values)
        if self.inform_label:
            s += ", inform_label: %s" % str(self.inform_label)
        if self.inform_slot_label:
            s += ", inform_slot_label: %s" % str(self.inform_slot_label)
        if self.refer_label:
            s += ", refer_label: %s" % str(self.refer_label)
        if self.diag_state:
            s += ", diag_state: %s" % str(self.diag_state)
        if self.class_label:
            s += ", class_label: %s" % str(self.class_label)
        return s
...
class InputFeatures(object):
    ...
    def __init__(self,
        ...
        self.refer_turn_id = refer_turn_id
        self.turn_ids = turn_ids

        self.refer_tokens = refer_tokens
        self.refer_marked = refer_marked
        self.refer_segment = refer_segment

        self.inform_tokens = inform_tokens
        self.inform_marked = inform_marked
        self.inform_segment = inform_segment
...
def convert_examples_to_features(examples, slot_list, class_types, model_type, tokenizer, max_seq_length,
    ...
    def _tokenize_text_and_label(text, text_label_dict, slot, tokenizer, model_specs, slot_value_dropout, text_pos):
        ...
        token_labels = []
        token_positions = []
        for token, token_label, joint_label, token_pos in zip(text, text_label, joint_text_label, text_pos):
            ...
            if slot_value_dropout == 0.0 or joint_label == 0:
                ...
                token_positions.extend([token_pos] * len(sub_tokens))
            else:
        ...
        return tokens, tokens_unmasked, token_labels, token_positions
    ...
    def _get_transformer_input(tokens_a, tokens_b, history, history_pos, max_seq_length, tokenizer, model_specs, model_type):
        ...
        for token in tokens_a:
            ...
            segment_ids.append(0 if model_type != "smallcustombert" else 1)
            turn_ids.append(0)
        ...
        segment_ids.append(1)
        turn_ids.append(1)
        if history_pos and history:
            for token, token_pos in zip(history, history_pos):
                tokens.append(token)
                segment_ids.append(1)
                turn_ids.append(token_pos + 1)
                # print(token_pos, "test")
            tokens.append(model_specs['SEP_TOKEN'])
            segment_ids.append(1)
            turn_ids.append(token_pos + 1)
        else:
        ...
        # print(segment_ids)
        return tokens, input_ids, input_mask, segment_ids, turn_ids
    ...
    for (example_index, example) in enumerate(examples):
        ...
        for slot in slot_list:
            tokens_a, tokens_a_unmasked, token_labels_a, token_a_pos = _tokenize_text_and_label(
                example.text_a, example.text_a_label, slot, tokenizer, model_specs, slot_value_dropout,
                [0] * len(example.text_a))
            tokens_b, tokens_b_unmasked, token_labels_b, token_b_pos = _tokenize_text_and_label(
                example.text_b, example.text_b_label, slot, tokenizer, model_specs, slot_value_dropout,
                [0] * len(example.text_b))
            tokens_history, tokens_history_unmasked, token_labels_history, tokens_history_pos = _tokenize_text_and_label(
                example.history, example.history_label, slot, tokenizer, model_specs, slot_value_dropout,
                example.history_pos if example.history_pos else [0] * len(example.history))
            ...
            refer_dict[slot] = example.refer_label[slot]
            ...
            if example.refer_turn_label is not None:
                refer_turn_dict[slot] = example.refer_turn_label[slot]
        ...
        tokens, input_ids, input_mask, segment_ids, turn_ids = _get_transformer_input(tokens_a,
                                                                                          tokens_b,
                                                                                          tokens_history,
                                                                                          tokens_history_pos,
                                                                                          max_seq_length,
        ...
        if slot_value_dropout > 0.0:
            _, input_ids_unmasked, _, _, _ = _get_transformer_input(tokens_a_unmasked,
                                                                    tokens_b_unmasked,
        ...
        # print(example.guid)
        # print(input_ids)
        # print(input_ids_unmasked)
        # print(input_mask)
        # print(segment_ids)
        # print(example.history_pos)
        # print(turn_ids)
        # print(example.turn_weight)
        # print(class_label_id_dict)
        # print(diag_state_dict)

        def truncate_tokens(tokens, max_len):
            marked = [1] * len(tokens)
            segment = [1] * len(tokens)
            while len(tokens) < max_len:
                tokens.append(0)
                marked.append(0)
                segment.append(0)
            tokens = tokens[:max_len]
            marked = marked[:max_len]
            segment = segment[:max_len]
            return tokens, marked, segment

        # print(inform_dict)
        refer_tokens = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(",".join([refer_dict[r] for r in refer_dict if refer_dict[r] != 'none']) + " [SEP]"))
        inform_tokens = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(",".join([r+":"+inform_dict[r] for r in inform_dict if inform_dict[r] != 'none']) + " [SEP]"))

        refer_tokens, refer_marked, refer_segment = truncate_tokens(refer_tokens, 32)
        inform_tokens, inform_marked, inform_segment = truncate_tokens(inform_tokens, 32)
        ...
        features.append(
            InputFeatures(
                ...
                turn=example.turn,
                turn_ids=turn_ids,
                max_turn=example.max_turn,
                turn_weight=example.turn_weight,
                refer_turn_id=refer_turn_dict,
                refer_tokens=refer_tokens,
                refer_marked=refer_marked,
                refer_segment=refer_segment,
                inform_tokens=inform_tokens,
                inform_marked=inform_marked,
                inform_segment=inform_segment))
